...
- **Phase D (Placement tự chấm ra CEFR — engine threshold):** ✅ code xong (PR này).
- **Phase E–F:** 🔜 chưa bắt đầu.
...
1. ⬜ **Kết nối Supabase + smoke test**: chạy `schema.sql` + `seed.sql` + `seed_placement.sql` → HS làm bài xếp lớp (ra CEFR) + luyện viết → GV chấm → xem chẩn đoán/tiến bộ.
...
### Phase D — Placement tự chấm ra CEFR ✅ (PR này)
