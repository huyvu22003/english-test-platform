...
1. ⬜ **Kết nối Supabase + smoke test toàn hệ**: `schema.sql` + `seed.sql` + `seed_placement.sql` → xếp lớp (CEFR) + luyện viết + buổi thi (mã) → GV chấm/chẩn đoán/tiến bộ.
2. 🔜 **Combine CEFR nhiều kỹ năng** → 1 trình độ tổng (placement gồm reading+listening+UoE).
3. 🔜 **Tách ngân hàng `items`** khỏi đề để tái dùng câu (refactor lớn).
4. 🔜 **Nội dung thật** Reading/Listening; xuất bảng điểm buổi thi; upload .xlsx trực tiếp.
5. 🔜 **Nội dung thật** Reading/Listening cho placement; mở rộng chẩn đoán theo `tag`.
