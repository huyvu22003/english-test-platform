...
2. ⬜ **Kết nối Supabase + smoke test toàn hệ**: `schema.sql` + `seed.sql` + `seed_placement.sql` → xếp lớp (CEFR) + luyện viết + buổi thi (mã) → GV chấm/chẩn đoán/tiến bộ.
3. 🔜 **Phase G1: vận hành thật cho trung tâm nhiều chi nhánh** — export bảng điểm buổi thi, chuẩn hóa chi nhánh/lớp/học viên, tài liệu vận hành.
4. 🔜 **Combine CEFR nhiều kỹ năng** → 1 trình độ tổng (placement gồm reading+listening+UoE).
5. 🔜 **Tách ngân hàng `items`** khỏi đề để tái dùng câu (refactor lớn).
6. 🔜 **Nội dung thật** Reading/Listening; xuất bảng điểm buổi thi; upload .xlsx trực tiếp.
