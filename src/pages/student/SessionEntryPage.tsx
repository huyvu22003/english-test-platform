...
export default function SessionEntryPage() {
  ...
  async function enter() {
    setErr(null);
    if (!ready) {
      setErr("Nhập mã thi, họ tên và email hợp lệ.");
      return;
    }
    ...
    try {
      ...
      if (!s) {
        setErr("Mã thi không đúng.");
        return;
      }
      if (s.status === "no_test") {
        setErr("Buổi thi chưa gắn đề.");
        return;
      }
      if (s.status === "not_open") {
        setErr(`Buổi thi chưa mở (mở lúc ${s.open_at ? new Date(s.open_at).toLocaleString("vi-VN") : "?"}).`);
        return;
      }
      if (s.status === "closed") {
        setErr(`Buổi thi đã đóng (${s.close_at ? new Date(s.close_at).toLocaleString("vi-VN") : ""}).`);
        return;
      }
      if (s.status === "class_mismatch") {
        setErr(
          `Email này không thuộc lớp được phép vào buổi thi${s.class_name ? ` (${s.class_name})` : ""}. Vui lòng kiểm tra email hoặc liên hệ giáo viên.`,
        );
        return;
      }
      nav(`/session/${s.session_id}`, {
        state: {
          ...
          testId: s.test_id,
          skill: s.skill,
          studentCode: current.code ?? null,
          studentMode: current.mode,
          sessionName: s.name,
          maxViolations: s.max_violations ?? 0,
          closeAt: s.close_at,
  ...
  return (
    <div className="wrap narrow session-entry-wrap">
      ...
      <div className="card session-entry-card">
        <label className="field session-code-field">
          <span>Mã thi</span>
          <input
            value={code}
        ...
        <div className="grid2">
          ...
          <label className="field">
            ...
            <input
              ...
              type="email"
              placeholder="email đã đăng ký lớp"
            />
          </label>
        </div>
        ...
        <button className="btn primary big" disabled={busy} onClick={enter}>
          {busy ? "Đang vào…" : "Vào thi"}
        </button>
      </div>
    </div>
  );
