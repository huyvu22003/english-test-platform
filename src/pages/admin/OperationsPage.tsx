...
import { listClasses, listClassTeachers, listProfiles, listSessions, listStudents, listSubmissions } from "../../lib/api";
...
function classNameForSubmission(s: Submission, students: Student[], classes: ClassRow[]): string {
  const student = students.find((x) => x.id === s.student_id || (!!x.email && !!s.student_email && x.email.toLowerCase() === s.student_email.toLowerCase()));
...
export default function OperationsPage() {
  ...
  const loading = subs.loading || students.loading || classes.loading || sessions.loading || profiles.loading || classTeachers.loading;
  ...
  const teacherWorkload = useMemo(() => {
    const teachers = (profiles.data ?? []).filter((p) => p.active !== false && ["owner", "admin", "teacher", "grader"].includes(p.role));
  ...
  const classRows = useMemo(() => {
    return (classes.data ?? []).map((c) => {
      ...
      const classSubs = allSubs.filter((s) => (!!s.student_id && ids.has(s.student_id)) || (!!s.student_email && emails.has(s.student_email.toLowerCase())));
      const classPending = classSubs.filter(isPending);
      const teachers = (classTeachers.data ?? [])
        .filter((x) => x.class_id === c.id)
        .map((x) => profiles.data?.find((p) => p.id === x.teacher_id)?.full_name || profiles.data?.find((p) => p.id === x.teacher_id)?.email || "GV")
        .join(", ");
      return {
        className: c.name,
  ...
  const errors = [subs.error, students.error, classes.error, sessions.error, profiles.error, classTeachers.error].filter(Boolean).join(" | ");
  ...
  return (
    <div className="admin-page operations-page">
      ...
      <section className="admin-stat-grid" aria-label="Tổng quan vận hành">
        <div className="admin-stat-card urgent"><span>Bài chờ xử lý</span><strong>{pending.length}</strong></div>
        <div className="admin-stat-card urgent"><span>Quá 36 giờ</span><strong>{overdue.length}</strong></div>
        <div className="admin-stat-card"><span>Học viên chưa xếp lớp</span><strong>{unassignedStudents}</strong></div>
        <div className="admin-stat-card"><span>Buổi thi đang mở</span><strong>{openSessions.length}</strong></div>
      </section>
      ...
      <div className="grid2 admin-split-grid">
        <section className="card admin-form-card ops-card">
          ...
          {teacherWorkload.length === 0 ? (
            <EmptyState title="Chưa có hồ sơ giáo viên" body="Sau khi tài khoản giáo viên đăng nhập hoặc profile được tạo, danh sách sẽ hiện tại đây." />
          ) : (
            <table className="table compact-table">
              <thead><tr><th>Giáo viên</th><th>Đang chấm</th><th>Đã chấm</th><th>Lớp</th></tr></thead>
              <tbody>{teacherWorkload.map((x) => (
                <tr key={x.profile.id}>
                  <td><strong>{x.profile.full_name || x.profile.email}</strong><div className="muted small">{x.profile.role}</div></td>
                  <td>{x.assigned}</td>
                  <td>{x.graded}</td>
        ...
        <section className="card admin-form-card ops-card">
          ...
          <ul className="ops-alert-list">
            <li><strong>{pending.length}</strong><span>bài chưa hoàn tất chấm/duyệt.</span></li>
            <li><strong>{overdue.length}</strong><span>bài đã chờ quá 36 giờ.</span></li>
            <li><strong>{unassignedStudents}</strong><span>học viên chưa được xếp lớp.</span></li>
            <li><strong>{noCodeStudents}</strong><span>học viên chưa có mã HV.</span></li>
            <li><strong>{openSessions.length}</strong><span>buổi thi đang mở.</span></li>
          </ul>
        </section>
      </div>
      ...
      <section className="card table-wrap ops-table-card">
        ...
        <table className="table">
          <thead><tr><th>Lớp</th><th>Giáo viên phụ trách</th><th>Học viên</th><th>Chờ chấm</th><th>Đã chấm</th><th>Band TB</th></tr></thead>
          <tbody>
            {classRows.map((r) => (
              <tr key={r.className}>
            ...
            {classRows.length === 0 && <tr><td colSpan={6} className="muted">Chưa có lớp để thống kê.</td></tr>}
          </tbody>
        </table>
      </section>
      ...
      {pending.length > 0 && (
        <section className="card table-wrap ops-table-card">
          ...
          <table className="table">
            <thead><tr><th>Nộp lúc</th><th>Học viên</th><th>Lớp</th><th>Chủ đề</th><th>Trạng thái</th></tr></thead>
            <tbody>{pending.slice(0, 12).map((s) => (
              <tr key={s.id}>
                ...
                <td>{s.topic_name}</td>
                <td><span className="pill off small">{s.status === "submitted" ? "Chờ chấm" : s.status === "assigned" ? "Đã giao" : "Cần review"}</span></td>
              </tr>
            ))}</tbody>
          </table>
        </section>
